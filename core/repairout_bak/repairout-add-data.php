...
if($act == 'add'){
	...
	$query = "UPDATE ".DB_PREFIX."repair_main SET repair_title = ?, repair_desc = ?,edit_date = ?,edit_users = ?,eq_id = ? ,eq_name = ?,eq_code = ?,approve_date = ? ,approve_username = ?, return_date = ? ,return_username = ?,repair_place = ? , repair_out = ?,repair_outdate = ? , repair_outcode = ? WHERE repair_id = ? LIMIT 1"; 
    ...
    $stmt->bindParam(13, $repair_out, PDO::PARAM_STR);
    $stmt->bindParam(14, $repairoutdate, PDO::PARAM_STR);
    $stmt->bindParam(15, $repair_outcode, PDO::PARAM_STR);
	$stmt->bindParam(16, $repairid, PDO::PARAM_INT);
	$stmt->execute();
