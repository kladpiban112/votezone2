...
function getUserDetailsArray($user_id)
{
    ...
    $UserDetailsArray = ['email' => $email, 'name' => $name, 'permalink' => $permalink, 'avatar' => $avatar, 'role_id' => $role_id, 'active' => $active, 'email_verified' => $email_verified, 'last_activity' => $last_activity, 'role' => $role, 'role_title' => $role_title, 'bio' => $bio, 'register_ip' => $register_ip, 'register_time' => $register_time, 'register_host' => $register_host, 'telephone' => $telephone, 'shortname' => $shortname];
...
function getCategDetailsArray($categ_id)
{
    ...
    $CategDetailsArray = ['title' => $categ_title, 'permalink' => $categ_permalink];
...
// สร้าง HN 
function getHn($orgid){
    global $conn;
    $yearnow = date("Y");
    $monthnow = date("m");


    $stmt = $conn->prepare('SELECT MAX(code_number) AS last_number FROM '.DB_PREFIX.'person_hn WHERE org_id = ? AND code_month = ? AND code_year = ? LIMIT 1 ');
    $stmt->execute([$orgid, $monthnow, $yearnow]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    $stmt = $conn->prepare('INSERT INTO '.DB_PREFIX.'person_hn (org_id,code_month,code_year,code_number)VALUE(?,?,?,?)');
    $stmt->execute([$orgid, $monthnow, $yearnow, $n]);
    
    $yearnowthai = substr($yearnow,2,2)+43;
    //$orgid_new = str_pad($orgid, 4, '0', STR_PAD_LEFT);
    $hn = $yearnowthai.$monthnow.$num;
...
function getPersonBorrowRequestAddr($personid){
    global $conn;
    if ($personid != '') {
        $stmt = $conn->prepare('SELECT o.*,c.changwatname,a.ampurname,t.tambonname
        FROM person_borrow_request o
        LEFT JOIN cchangwat c ON o.changwat = c.changwatcode
        LEFT JOIN campur a ON CONCAT(o.changwat,o.ampur) = a.ampurcodefull
        LEFT JOIN ctambon t ON CONCAT(o.changwat,o.ampur,o.tambon) = t.tamboncodefull
        WHERE o.oid = ? ');
...
function getPersonBorrowAddr($personid){
    global $conn;
    if ($personid != '') {
        $stmt = $conn->prepare('SELECT o.*,c.changwatname,a.ampurname,t.tambonname
        FROM person_borrow o
        LEFT JOIN cchangwat c ON o.changwat = c.changwatcode
        LEFT JOIN campur a ON CONCAT(o.changwat,o.ampur) = a.ampurcodefull
        LEFT JOIN ctambon t ON CONCAT(o.changwat,o.ampur,o.tambon) = t.tamboncodefull
        WHERE o.oid = ? ');
