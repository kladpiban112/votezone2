...
$repair_placeto = filter_input(INPUT_POST, 'repair_placeto', FILTER_SANITIZE_STRING); // 
...
if($act == 'add'){
	...
	$query = "UPDATE ".DB_PREFIX."repair_main SET repair_title = ?, repair_desc = ?,edit_date = ?,edit_users = ?,eq_id = ? ,eq_name = ?,eq_code = ?,approve_date = ? ,approve_username = ?, return_date = ? ,return_username = ?,repair_place = ? , repair_warranty = ?,repair_placeto = ? WHERE repair_id = ? LIMIT 1"; 
    ...
    $stmt->bindParam(13, $repair_warranty, PDO::PARAM_STR);
    $stmt->bindParam(14, $repair_placeto, PDO::PARAM_STR);
