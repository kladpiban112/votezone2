...
$repair_refid = filter_input(INPUT_POST, 'repair_refid', FILTER_SANITIZE_STRING);
...
if($exist_person!=0){
	...
	// AVATAR
		...
		$repair_code = getRepairoutcode($org_id,$repairdate);  
		$repair_code = "O".$repair_code;
		$query = "INSERT INTO ".DB_PREFIX."repair_main (repair_id,person_id,org_id, repair_date,repair_code, repair_type,flag,add_date,add_users,repair_place,repair_inout,repair_refid) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?,?,?,?,?)"; 
		$stmt = $conn->prepare($query);
		...
		$stmt->bindParam(11, $repair_refid, PDO::PARAM_STR);
		$stmt->execute();
