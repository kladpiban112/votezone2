...
$query = 'UPDATE '.DB_PREFIX.'repair_spare SET spare_name = ?,spare_quantity = ?,spare_unit = ?,spare_price = ?,spare_desc =?, edit_date = ?,edit_users = ? WHERE oid = ? ';
$stmt = $conn->prepare($query);

$stmt->bindParam(1, $spare_id, PDO::PARAM_STR);
$stmt->bindParam(2, $spare_quantity, PDO::PARAM_STR);
$stmt->bindParam(3, $spare_unit, PDO::PARAM_STR);
$stmt->bindParam(4, $spare_price, PDO::PARAM_STR);
$stmt->bindParam(5, $spare_desc, PDO::PARAM_STR);
$stmt->bindParam(6, $now, PDO::PARAM_STR);
$stmt->bindParam(7, $logged_user_id, PDO::PARAM_STR);
$stmt->bindParam(8, $spareid, PDO::PARAM_STR);
