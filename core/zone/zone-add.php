...
if($act == 'add'){
	...
	$query = "INSERT INTO ".DB_PREFIX."area (area_number, changwat, ampur, tambon,village, zone_number, zone_name, details, zone_code) VALUES (?, ?, ?, ?, ?, ?, ?, ?,?)"; 
	...
	$stmt->bindParam(9, $zone_code, PDO::PARAM_STR);
	$stmt->execute();
	...
	$query = "UPDATE  ".DB_PREFIX."area SET  area_number = ?, changwat = ?, ampur = ?,tambon = ?,village = ?,zone_number = ?,zone_name = ? ,latitude = ? ,longitude = ? ,details = ? WHERE aid = ? LIMIT 1"; 
	$stmt = $conn->prepare($query);
	$stmt->bindParam(1, $area_number, PDO::PARAM_STR);
	$stmt->bindParam(2, $changwat, PDO::PARAM_STR);
	$stmt->bindParam(3, $ampur, PDO::PARAM_STR);
	...
	$stmt->bindParam(10, $details, PDO::PARAM_STR);
	$stmt->bindParam(11, $aid, PDO::PARAM_STR);
	$stmt->execute();
