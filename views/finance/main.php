...
?>
		...
		<div class="card card-custom gutter-b example example-compact">
			<div class="card-header">
				<h3 class="card-title">
                <i class="fas fa-file-invoice-dollar"></i>&nbsp;ข้อมูลการชำระเงิน 
				</h3>
    ...
    <div class="form-group row">
                                          ...
                                          <select class="form-control " name="status" id="status">
                                                      ...
                                                      <?php
                                                        ...
                                                        while ($row = $stmt_user_role->fetch(PDO::FETCH_ASSOC))
                                                          ...
                                                          ?>
                                                          <option value="<?php echo $id_selected;?>" <?php if($status == $id_selected) { echo "selected";}?>><?php echo $title_selected;?></option>
                                                          <?php
                                                          }
                                                        ?>
                                          </select>
            ...
            <div class="col-lg-2">
                <label>สถานะชำระเงิน</label>
                                          <select class="form-control " name="payment_success" id="payment_success">
                                                      <option value="" <?php if($payment_success == ''){echo "selected";}?>>ทั้งหมด</option>
                                                      <option value="0" <?php if($payment_success == '0'){echo "selected";}?>>รอชำระเงิน</option>
                                                      <option value="1" <?php if($payment_success == '1'){echo "selected";}?>>ชำระครบแล้ว</option>
    ...
    INNER JOIN ".DB_PREFIX."repair_payment pm ON u.repair_id = pm.repair_id
    WHERE u.flag != 0  AND pm.flag = '1' $conditions  $search_data $repairdate_data $status_data $payment_success_data  ";    
    $numb_data = $conn->query($sql)->fetchColumn();
        ...
        $max = ' LIMIT ' .($pagenum - 1) * $page_rows .',' .$page_rows;	
         ...
         $sql = "SELECT u.*,p.*,o.org_shortname ,t.repair_typetitle,if(e.eq_code IS NOT NULL ,e.eq_code,u.eq_code) AS eq_code, if(e.eq_name IS NOT NULL ,e.eq_name,u.eq_name) AS eq_name,st.status_title,pm.cost,pm.cost_payment,pm.cost_success
...
<div class="table-responsive">
    ...
    <tbody>
            ...
            while ($row = $stmt_data->fetch(PDO::FETCH_ASSOC))
            {
                ...
                $repair_out = $row['repair_out'];
                if($repair_out == 'I'){
                    $repair_out_show = "<i class='fas fa-tools text-success'></i>";
                }else{
                    $repair_out_show = "<i class='fas fa-truck text-danger'></i>";
                }
                ...
                if(($repair_status == '3') AND ($return_date != "")){
                    ...
                    if($cost_success == '0'){
                        $cost_success_status = "<i class='fas fa-exclamation-circle text-warning'></i>";
                    }else{
                        $cost_success_status = "<i class='fas fa-check-circle text-success'></i>";
                    }
                ?>
                <tr>
                            ...
                            <td class="text-center"><?php echo $cost_success_status;?></td>
                            <td><?php echo $cost;?></td>
                            ...
                            <!--begin::Dropdown-->
                                <div class="dropdown">
                                    ...
                                    <ul class="navi navi-hover py-1">
                                        ...
                                        <li class="navi-item">
                                            <a href="dashboard.php?module=finance&page=payment-repair-add&repairid=<?php echo $repairid_enc;?>&personid=<?php echo $personid_enc;?>&act=<?php echo base64_encode('edit');?>" class="navi-link">
                                                <span class="navi-icon"><i class="fas fa-cash-register"></i></span>
...
$Prev_Page = $pagenum-1;
$Next_Page = $pagenum+1;
$page_link = "dashboard.php?module=$module&page=main&search=$search&status=$status&startdate=$startdate&stopdate=$stopdate&payment_success=$payment_success&pagenum";
