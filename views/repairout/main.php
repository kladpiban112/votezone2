...
?>
		...
		<div class="card card-custom gutter-b example example-compact">
			<div class="card-header">
				<h3 class="card-title">
                <i class="fas fa-paper-plane text-info"></i>&nbsp;ข้อมูลการส่งซ่อมภายนอก
				</h3>
				<div class="card-toolbar">
					<div class="example-tools justify-content-center">
						<a href="dashboard.php?module=repairout&page=repairout-add" class="btn btn-success btn-sm font-weight-bold mr-2" title="เพิ่มรายการแจ้งซ่อม"><i class="fa fa-plus-circle" title="เพิ่มรายการส่งซ่อมภายนอก" data-toggle="tooltip"></i>บันทึกส่งซ่อม</a>
    ...
    <?php
       if($logged_user_role_id == '1'){
        $conditions = " AND u.repair_inout = 'O' ";
        }else{

        $conditions = " AND u.org_id = '$logged_org_id'  AND u.repair_inout = 'O'  ";
        }
    ...
    WHERE u.flag != 0  $conditions  $search_data $repairdate_data $status_data $refid_data ")->fetchColumn();
        ...
        $sql = "SELECT u.*,p.*,o.org_shortname ,t.repair_typetitle,if(e.eq_code IS NOT NULL ,e.eq_code,u.eq_code) AS eq_code, if(e.eq_name IS NOT NULL ,e.eq_name,u.eq_name) AS eq_name,st.status_title,r.repair_code AS repair_refcode
...
<div class="table-responsive">
    ...
    <tbody>
            ...
            while ($row = $stmt_data->fetch(PDO::FETCH_ASSOC))
            {
                ...
                $repair_refid = $row['repair_refid'];
                $repair_refid_enc = base64_encode($repair_refid);
                $repair_refcode = $row['repair_refcode'];
                ...
                if(($repair_status == '3') AND ($return_date != "")){
                    ...
                    $repair_outcode = $row['repair_outcode'];
                    $repair_outdate = date_db_2form($row['repair_outdate']);
                ...
                if ($qt_status == '2') {
                    ...
                    $qt_status_show = "<i class='fas fa-time text-danger'></i>";
                } else {
                    $qt_status_show = '';
                } 
                ?>
                <tr>
                            ...
                            <td class="text-center"><?php echo $repair_code;?></br><small>Ref : <a href="dashboard.php?module=repair&page=repair-add-data&repairid=<?php echo $repair_refid_enc;?>&personid=<?php echo $personid_enc;?>&act=<?php echo base64_encode('edit');?>" ><?php echo $repair_refcode; ?></a></small></td>
                                    ...
                                    <ul class="navi navi-hover py-1">
                                        ...
                                        <li class="navi-item">
                                            <a href="dashboard.php?module=repairout&page=repairout-add&repairid=<?php echo $repairid_enc;?>&personid=<?php echo $personid_enc;?>&act=<?php echo base64_encode('edit');?>" class="navi-link">
                                                ...
                                                <span class="navi-text">แก้ไขข้อมูลส่งซ่อม</span>
                                            </a>
                                        </li>
