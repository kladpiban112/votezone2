...
$refid_enc = filter_input(INPUT_GET, 'refid', FILTER_SANITIZE_STRING);
...
if($action == "edit"){
    ...
    $refid = $row_service['repair_refid'];
    $repair_date = date_db_2form($row_service['repair_date']);
    ...
    $sql_service = "SELECT s.* FROM ".DB_PREFIX."repair_main s 
	LEFT JOIN ".DB_PREFIX."person_main p ON s.person_id = p.oid 
    WHERE s.repair_id = '$refid' AND s.flag != '0'  LIMIT 1";
    $stmt_service = $conn->prepare ($sql_service);
    $stmt_service->execute();	
	$row_service = $stmt_service->fetch(PDO::FETCH_ASSOC);
...
?>
      ...
      <div class="ribbon-target bg-danger" style="top: 10px; right: -2px;">1</div>
				<h3 class="card-title">
                <i class="fas fa-paper-plane text-info"></i>&nbsp;<?php echo $txt_title;?>รายการส่งซ่อมภายนอก
				</h3>
...
<input type="hidden" class="form-control"  name="repair_inout" id="repair_inout" value="O"/>
<input type="hidden" class="form-control"  name="org_id" id="org_id" value="<?php echo $logged_org_id;?>"/>
    ...
    <span><i class="far fa-bookmark"></i> เลขที่แจ้งซ่อม</span>
  <hr>   
    ...
    <span><i class="far fa-bookmark"></i> ข้อมูลส่งซ่อมซ่อม</span>
  <hr>   
    <div class="form-group row">

    <div class="col-lg-2">
				<label>วันที่ส่งซ่อม</label>
				<input type="text" class="form-control"  name="repairdate" id="repairdate" placeholder="วันที่ส่งซ่อม" value="<?php echo $repair_date;?>"  data-date-language="th-th" maxlength="10" />
	...
	<div class="card-footer">
		<div class="row">
			<div class="col-lg-6">
                ...
                <?php if($action == "edit"){?>
                <a href = "dashboard.php?module=repairout&page=repairout-add-data&repairid=<?php echo $repairid_enc;?>&personid=<?php echo $personid_enc;?>&act=<?php echo base64_encode('edit');?>" type="button" class="btn btn-warning" o><i class="fa fa-chevron-right" title="ถัดไป" ></i> </a>
                <?php } ?>
			</div>
...
$("#repair_refid").change(function() {
    var repair_refid = btoa($("#repair_refid").val());
    window.location.replace("dashboard.php?module=repairout&page=repairout-add&refid="+repair_refid);
...
$('#btnSave').click(function(e){
        ...
        $.ajax({
            type: "POST",
            url: "core/repairout/repairout-add.php",
            dataType: "json",
			data: data,
			processData: false,
            contentType: false,
