...
$oid_team = filter_input(INPUT_GET, 'oid', FILTER_SANITIZE_STRING);
...
$person_N = " AND p.level = 5 AND  p.head = '$oid_team' AND p.team_id = '$team_id' ";
?>
		<!--begin::Card-->
    ...
    $numb_data = $conn->query("SELECT count(1) FROM ".DB_PREFIX."person_main p  WHERE p.flag != '0' $conditions  $search_data    $cid_data  $slevel_data $cchangwat_data $campur_data $ctambon_data $person_N")->fetchColumn();
        ...
        WHERE p.flag != '0' $conditions  $search_data  $cid_data  $slevel_data $cchangwat_data $campur_data $ctambon_data $person_N
