...
if($action == "edit"){
	...
	WHERE u.user_id = '$userid' AND u.active = '1'  LIMIT 1");
	$stmt_data->execute();	
	...
	<div class="col-lg-9">
		...
		<div class="form-group row">
			...
			<div class="col-lg-4">
				...
				<span class="form-text text-muted">เว้นว่างไว้ กรณีที่ไม่ต้องการเปลี่ยน</span>
			</div>
		...
		<div class="col-lg-3 border-x-0 border-x-md border-y border-y-md-0">
				...
				<div class="form-group row">
					<div class="col-lg-12">
						...
						<select class="form-control "  name="user_role" id="user_role" disabled>
							<option value="">ระบุ</option>
							<?php
							if($logged_user_role_id == '1'){
								$conditions = " ";
					...
					<div class="col-lg-12">
						...
						<select class="form-control "  name="user_status" id="user_status" disabled>
							<option value="1" <?php if($row['active'] == '1' ){echo "selected";} ?>>เปิดใช้งาน</option>
							<option value="0" <?php if($row['active'] == '0' ){echo "selected";} ?>>ปิดใช้งาน</option>
						</select>
					</div>
				</div>
				</div>
