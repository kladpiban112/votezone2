...
<head>
    <meta charset="utf-8">
    <title>invoice/ใบเสร็จรับเงิน</title>
...
,p.payment_pricevat,p.payment_price,p.payment_advince,p.payment_pay,u.name
...
<body>
    <div class="invoice-box">
        ...
        <tr>
            ...
            <tr class="details">
                <td colspan=2>
                    ที่อยู่ :  <?php echo $booking_custaddress;?>
                </td>
            ...
            <tr class="item">
                <td colspan=2>
                    ช่วงเวลา :  <?php echo date_sub_thai($booking_checkin);?> ถึง <?php echo date_sub_thai($booking_checkout);?> จำนวน <?php echo $s_booking_amountday; ?> คืน
                </td>
            </tr>
            ...
            <tr class="item">
                <td colspan=2>
                   ประเภทห้อง : <?php echo $rooms_typetitle; ?>(<?php echo $bed_title;?>) จำนวนห้อง 1 ห้อง
                </td>
            ...
            <tr class="item">
                <td>
                    ค่าห้องพักทั้งหมด <?php echo $booking_price." X ".$s_booking_amountday; ?> 
                </td>
...
//$number_additional = $stmt_additional->fetchColumn(); 
            ...
            <?php
            echo $sql_pdetail = "SELECT * FROM ".DB_PREFIX."payment_detail WHERE payment_bookingid = '$bookingid'";
            $stmt_pdetail = $conn->prepare ($sql_pdetail);
            $stmt_pdetail->execute();
            $nRows = $stmt_pdetail->rowCount(); 
            $spprice = 0;
            if($nRows > 0){
                    $i=1;
                    while ($rs = $stmt_pdetail->fetch(PDO::FETCH_ASSOC))
                    {
                        $pmethod = $rs['payment_method'];
                        $pnote = $rs['payment_note'];
                        $ppay = $rs['payment_pay'];

                        $spprice += $ppay;

                        if($pmethod == '1') { $pmethodname = 'เงินสด'; 
                        }elseif($pmethod == '2') { $pmethodname = 'โอนเงิน'; 
                        }
              ...
              echo  "<tr class='item'><td>".$pmethodname.$pnote_show."(".ThaiBahtConversion($ppay).")</td><td>".$ppay."</td></tr>";
            ...
            <tr class="total">
                ...
                <td>
                คงเหลือ : <?php echo number_format($booking_pricetotal-$spprice,2);?>
                </td>
            </tr>
            <?php
                    }
